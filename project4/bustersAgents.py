...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    liveGhostsPos = [max(ghost_pos.items(), key=lambda x: x[1]) for ghost_pos in livingGhostPositionDistributions]
    bestActions = [min(legal, key=lambda x: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, x), ghost_pos[0])) for ghost_pos in liveGhostsPos]
    #print bestActions[0]
