...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    "*** YOUR CODE HERE ***" #Done - Old code + small modifications from the comment
    ...
    if noisyDistance == None:
        ...
        for p in self.legalPositions:
            ...
            allPossible[p] = emissionModel[trueDistance] * self.beliefs[p]  #Beliefs are stored as util.Counter objects (like dictionaries) in a field called self.beliefs
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***" #Done - Most of the code from the above comments
    allPossible = util.Counter()
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance is None:
        self.particles = [self.getJailPosition() for i in range (0, len(self.particles))]
    else:
        for p in self.particles:
            dist = util.manhattanDistance(pacmanPosition, p)
            allBeliefs[p] += emissionModel[dist]

        if allBeliefs.totalCount() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [util.sample(allBeliefs) for i in range (0, len(self.particles))]
  ...
  def elapseTime(self, gameState):
    ...
    """
    "*** YOUR CODE HERE ***"
    self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))) for i in range (0, len(self.particles))]
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    #util.raiseNotDefined()
    self.particles = []
    iterout=itertools.product(self.legalPositions,repeat=self.numGhosts )
    postions_permutation=list(iterout)
    
    for i in range(0, self.numParticles):
        self.particles.append(postions_permutation[i % len(postions_permutation)])
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      for i in range(self.numGhosts):
        ...
        samplefromnewPosDist=util.sample(newPosDist)
        generatedparticle.append(samplefromnewPosDist)


      #newParticles.append(tuple(newParticle))
      newParticles.append(tuple(generatedparticle))
  ...
  def observeState(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    allBeliefs = util.Counter()
    #print "emmodel len=",len(emissionModels)," conten=",emissionModels
    ...
    for oldParticle in self.particles:
        ...
        for i in range(self.numGhosts):
            if noisyDistances[i] is None:
                ...
                dist = util.manhattanDistance(pacmanPosition, newParticle[i])
                weight = weight*emissionModels[i][dist]

        allBeliefs[tuple(newParticle)]=allBeliefs[tuple(newParticle)]+weight
        #newParticles.append(tuple(newParticle))
    allBeliefs.normalize()

    if allBeliefs.totalCount() == 0:
        self.initializeParticles()   ######## something not understood on the comment above(last line)
